...
class TestExcelReader(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.excel_mapper = CONST.FILE.YAML[CONST.TYPE.EXC]
    ...
    def create_xl(self, filename, oneperson=None, n_rows=10):
        headers = list(self.excel_mapper.values())

        data = defaultdict(list)
        for row in range(n_rows):
            for header in headers:
                if oneperson is not None and header == self.excel_mapper["matricule"]:
                    value = oneperson
                elif header == self.excel_mapper["libelle"]:
                    value = "DEPLACEMENT"
                else:
                    value = f"{row} {header}"
    ...
    def test_read(self):
        ...
        records_manager, status = Reader(self.filename)

        # Check the dict info
        # 1 person with matricule ALBERT and 10 missions
        self.assertEqual(len(records_manager), 1)
        self.assertEqual(len(records_manager._records[oneperson].missions), 10)
    ...
    def test_read_big(self):
        ...
        # Check the dict info
        self.assertEqual(len(records_manager), 1)
        self.assertEqual(len(records_manager._records[oneperson].missions), 1000)
    ...
    def test_read_change_matricule(self):
        ...
        # Check the dict info
        self.assertEqual(len(records_manager), 10)
        for record in records_manager:
